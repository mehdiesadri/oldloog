...
class MessageTestCase(TestCase):
    ...
    def test_message_user(self):
        msg = MessageModel.objects.create(
            user=self.test_user2, recipient=self.test_user1, body="test"
        )
        self.assertEqual(msg.user.username, "milhouse")

    def test_message_body(self):
        msg = MessageModel.objects.create(
            user=self.test_user1, recipient=self.test_user2, body="123"
        )
        self.assertEqual(msg.body, "123")

    def test_message_characters(self):
        msg = MessageModel.objects.create(
            user=self.test_user1, recipient=self.test_user2, body="a"
        )
        self.assertEqual(msg.characters(), 1)

    def test_message_body_strip(self):
        msg = MessageModel.objects.create(
            user=self.test_user1, recipient=self.test_user2, body=" aaa "
        )
    ...
    def test_message_create_retrieve(self):
        mid = MessageModel.objects.create(
            user=self.test_user1, recipient=self.test_user2, body="tbody"
        ).id
